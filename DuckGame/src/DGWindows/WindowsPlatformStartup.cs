...
internal class WindowsPlatformStartup
{
		...
		if (LoadLibrary(pLibrary) == IntPtr.Zero)
		{
			...
			default:
				try
				{
					...
					SearchPath(null, pLibrary, null, 255, buffer, out ptr);
		...
		try
		{
			...
			if (ntDll != IntPtr.Zero)
			{
				...
				if (pwine_get_version != IntPtr.Zero)
				{
					...
					IntPtr wineVersionPtr = ((wine_version_delegate)Marshal.GetDelegateForFunctionPointer(pwine_get_version, typeof(wine_version_delegate)))();
	...
	public static void UnhandledExceptionTrapper(object sender, UnhandledExceptionEventArgs e)
	{
		if (File.Exists("CrashWindow.exe"))
		{
			...
			catch (Exception pException)
			{
				...
				StreamWriter streamWriter = new StreamWriter("ducklog.txt", append: true);
	...
	public static List<Module> CollectModules(Process process)
	{
		...
		int bytesNeeded = 0;
		if (!Native.EnumProcessModulesEx(process.Handle, modulePointers, 0, out bytesNeeded, 3u))
		{
			return collectedModules;
		}
		int totalNumberofModules = bytesNeeded / IntPtr.Size;
		modulePointers = new IntPtr[totalNumberofModules];
		if (Native.EnumProcessModulesEx(process.Handle, modulePointers, bytesNeeded, out bytesNeeded, 3u))
		{
			for (int index = 0; index < totalNumberofModules; index++)
			{
				...
				Module module = new Module(moduleFilePath.ToString(), moduleInformation.lpBaseOfDll, moduleInformation.SizeOfImage);
