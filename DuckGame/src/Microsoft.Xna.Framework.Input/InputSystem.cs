...
public static class InputSystem
{
    ...
    public static bool AltDown
    {
        get
        {
            ...
            {
                return state.IsKeyDown(Keys.RightAlt);
            }
            return true;
        }
    }
    ...
    public static void Initialize(GameWindow window)
    {
        ...
        prevWndProc = SetWindowLongW(window.Handle, -4, (int)Marshal.GetFunctionPointerForDelegate(hookProcDelegate));
        hWND = window.Handle;
    }
    ...
    {
        ImmAssociateContext(hWND, hIMC);
        ImmSetOpenStatus(hIMC, open: true);
        ImmReleaseContext(hWND, hIMC);
    }
    ...
    {
        ImmAssociateContext(hWND, hIMC);
        ImmSetOpenStatus(hIMC, open: false);
        ImmReleaseContext(hWND, hIMC);
    }
    ...
    private static IntPtr HookProc(IntPtr hWnd, uint msg, IntPtr wParam, IntPtr lParam)
    {
        ...
        IntPtr returnCode = CallWindowProcW(prevWndProc, hWnd, msg, wParam, lParam);
        if (msg == 28)
        {
            if (wParam == IntPtr.Zero)
            {
                MonoMain.framesSinceFocusChange = 0;
            }
            else
            {
                MonoMain.framesSinceFocusChange = 0;
        ...
        if (Options.Data.imeSupport)
        {
            ...
            {
                case 135u:
                    returnCode = (returnCode.ToInt32() | 4);
                    break;
                case 256u:
                ...
                case 257u:
                    ...
                    break;
                case 646u:
                    IMECharEntered?.Invoke(null, new CharacterEventArgs((char)(int)wParam, lParam.ToInt32()));
                    break;
                case 641u:
