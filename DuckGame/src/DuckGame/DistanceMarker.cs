...
public class DistanceMarker : Thing
{
	...
	{
		_distanceSign.depth = base.depth;
		Graphics.Draw(_distanceSign, base.x, base.y);
		string text = Change.ToString(_distance);
		_font.Draw(text, base.x - _font.GetWidth(text) / 2f, base.y - _font.height / 2f + 1f, Color.DarkSlateGray, base.depth + 1);
	}
}
