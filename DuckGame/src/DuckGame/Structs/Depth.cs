...
public struct Depth
{
    ...
    {
        return new Depth(value + kSpanIncrement / 20f * (float)val, span);
    }

    public static implicit operator Depth(float val)
    {
        return new Depth(val);
    }

    public static Depth operator +(Depth c1, int c2)
    {
        return c1.Add(c2);
    }
    ...
    public static bool operator <(Depth c1, float c2)
    {
        return c1.value < c2;
    }
