...
public class ModConfiguration
{
	...
	public Assembly assembly
	{
		...
		internal set
		{
			_assembly = value;
			loaded = true;
			AssemblyTitleAttribute titleAttrib = GetCustomAttribute<AssemblyTitleAttribute>();
			if (titleAttrib != null)
			{
				displayName = titleAttrib.Title;
			}
			else
			{
				displayName = name;
			...
			AssemblyDescriptionAttribute descAttrib = GetCustomAttribute<AssemblyDescriptionAttribute>();
			if (descAttrib != null)
			{
				description = descAttrib.Description;
			}
			AssemblyCompanyAttribute authorAttrib = GetCustomAttribute<AssemblyCompanyAttribute>();
			if (authorAttrib != null)
			{
				author = authorAttrib.Company;
			}
			else
			{
				author = "Unknown";
	...
	public void SetModType(Type pType)
	{
		modType = pType;
	}

	public void SetMapPack(MapPack pPack)
	{
		_mapPack = pPack;
	}
	...
	{
		object[] customAttributes = _assembly.GetCustomAttributes(typeof(T), inherit: false);
		int num = 0;
		if (num < customAttributes.Length)
		{
			return (T)customAttributes[num];
		}
		return null;
	}
	...
	internal void LoadConfiguration()
	{
		...
		if (File.Exists(configFilePath))
		{
			...
			XmlElement modElement = configDocument.DocumentElement;
			XmlElement varSoftDependencies = modElement["SoftDependencies"];
			if (varSoftDependencies != null)
			{
				softDependencies = varSoftDependencies.InnerText.Split(new char[1] { '|' }, StringSplitOptions.RemoveEmptyEntries);
			}
			XmlElement varHardDependencies = modElement["HardDependencies"];
			if (varHardDependencies != null)
			{
				hardDependencies = varHardDependencies.InnerText.Split(new char[1] { '|' }, StringSplitOptions.RemoveEmptyEntries);
			}
			noCompilation = ((modElement["NoCompilation"] != null && modElement["NoCompilation"].InnerText.ToLower() == "true") ? true : false);
			preloadContent = ((modElement["PreloadContent"] == null || !(modElement["PreloadContent"].InnerText.ToLower() == "false")) ? true : false);
			processPinkTransparency = ((modElement["PinkTransparency"] == null || !(modElement["PinkTransparency"].InnerText.ToLower() == "false")) ? true : false);
			workshopID = ((modElement["WorkshopID"] != null) ? ulong.Parse(modElement["WorkshopID"].InnerText) : 0);
			majorSupportedRevision = ((modElement["MajorSupportedRevision"] != null) ? int.Parse(modElement["MajorSupportedRevision"].InnerText) : 0);
		}
		else
		{
			configDocument = new XmlDocument();
	...
	internal void Disable()
	{
		if (!disabled)
		{
			...
			disabled = true;
			ModLoader.SetModDisabled(this, pDisabled: true);
		}
	}
	...
	internal void Enable()
	{
		...
		{
			disabled = false;
			ModLoader.SetModDisabled(this, pDisabled: false);
		}
	}
	...
	{
		XmlElement modElement = configDocument.DocumentElement;
		if (modElement["WorkshopID"] == null)
		{
			modElement.AppendChild(configDocument.CreateElement("WorkshopID"));
		}
		modElement["WorkshopID"].InnerText = id.ToString();
		workshopID = id;
		configDocument.Save(configFilePath);
	}
}
