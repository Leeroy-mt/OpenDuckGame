...
public class FancyBitmapFont : Transform
{
    ...
    //public void Draw2(string text, float xpos, float ypos, Color c, Depth deep = default(Depth), bool colorSymbols = false) //fna batch test
    //{
    //    _previousColor = c;
    //    if (string.IsNullOrWhiteSpace(text))
    //    {
    ...
    //                        yCenter = (float)characterHeight * base.scale.y / 2f - (float)spr.height * spr.scale.y / 2f;
				//			spr.x = xpos + xOff;
				//			spr.y = ypos + yCenter;
				//			spr.depth = deep + 10 + (int)((ypos + yOff) / 10f);
    //                        Graphics.Draw(spr);
    //                        xOff += (float)spr.width * spr.scale.x + 1f;
    //                        spr.scale = sprScale;
    //                    }
    ...
    //                        _kanjiSprite.color = c;
    //                        _kanjiSprite.alpha = base.alpha;
				//			_kanjiSprite.x = xpos + xOff + 1f;
				//			_kanjiSprite.y = ypos + yOff + 1f;
				//			_kanjiSprite.depth = deep;
    //                        Graphics.Draw(_kanjiSprite);
    //                        xOff += 8f * base.scale.x;
    //                    }
    //                    else
    //                    {
