...
public class BitmapFont : Transform
{
    ...
    //public void Draw2(string text, float xpos, float ypos, Color c, Depth deep = default(Depth), InputProfile input = null, bool colorSymbols = false) //fna batch test
    //{
    //    if (colorOverride != default(Color))
    //    {
    //        c = colorOverride;
    ...
    //                fontTexture.color = c;
    //                fontTexture.alpha = base.alpha;
				//	fontTexture.x = xpos + xOff;
				//	fontTexture.y = ypos + yOff + characterYOffset;
				//	fontTexture.depth = deep;
    //                Graphics.Draw(fontTexture);
    //                xOff += (float)_tileSize * base.scale.x;
    //            }
    //        }
    //    }
