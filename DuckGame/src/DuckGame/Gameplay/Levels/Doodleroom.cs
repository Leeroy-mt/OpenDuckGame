...
internal class Doodleroom : Level
{
    ...
    public override void PostDrawLayer(Layer layer)
    {
        if (layer == Layer.HUD && _data != null)
        {
            ...
            Graphics.DrawString(_data.name, new Vector2(layer.camera.width / 2f - nameWidth / 2f, layer.camera.height / 2f - 43f), Color.Black, 1f);
            string text = "\"" + _data.quote + "\"";
            float quoteWidth = Graphics.GetStringWidth(text);
            Graphics.DrawString(text, new Vector2(layer.camera.width / 2f - quoteWidth / 2f, layer.camera.height / 2f + 38f), Color.Black, 1f);
        }
