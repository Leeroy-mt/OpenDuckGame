...
public class ScriptStatement
{
    ...
    public object result
    {
        get
        {
            ...
            leftResult = ((!(leftObject is ScriptStatement leftStatement)) ? leftObject : leftStatement.result);
            ScriptStatement rightStatement = rightObject as ScriptStatement;
            rightResult = ((rightStatement == null) ? rightObject : ((!(leftResult is bool) && op > ScriptOperator.IsNotEqual) ? null : rightStatement.result));
            if (leftResult != null)
            {
                if (rightResult != null)
                {
            ...
            if (functionName != null)
            {
                ...
                object res = null;
                res = ((finalResult is string) ? Script.CallMethod(functionName, finalResult as string) : ((finalResult is int) ? Script.CallMethod(functionName, (int)finalResult) : ((!(finalResult is float)) ? Script.CallMethod(functionName, null) : Script.CallMethod(functionName, (float)finalResult))));
                obj = res as ScriptObject;
                finalResult = ((obj == null) ? res : obj.objectProperty.GetValue(obj.obj, null));
    ...
    public static ScriptStatement Parse(string statement, object left = null, object right = null, ScriptOperator operat = ScriptOperator.None, string func = null, bool isChild = false)
    {
        ...
        while (statement.Length > 0)
        {
            ...
            else if (c == ' ' || c == '(' || c == ')')
            {
                ...
                if (c == '(')
                {
                    ScriptStatement innerStatement = Parse(statement, null, null, ScriptOperator.None, isWord ? currentWord : null, isChild: true);
                ...
                if (newStatement.data != null)
                {
                    ...
                    {
                        statement = ((ScriptStatement)(dat = Parse(statement, dat, null, ScriptOperator.None, null, isChild: true))).remainingStatementString;
                    }
                    ...
                    else
                    {
                        ...
                        {
                            return Parse(statement, newStatement, dat, currentOperator, null, isChild: true);
                        }
