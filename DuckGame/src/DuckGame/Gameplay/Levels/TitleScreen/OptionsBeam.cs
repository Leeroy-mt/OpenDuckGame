...
public class OptionsBeam : MaterialThing
{
    ...
    public override void Update()
    {
        ...
        for (int i = 0; i < _ducks.Count; i++)
        {
            ...
            {
                d2.duck.X = Lerp.FloatSmooth(d2.duck.X, X + (float)d2.sin2, 0.2f);
        ...
        for (int i2 = 0; i2 < _guns.Count; i2++)
        {
            ...
            g.vSpeed = 0;
            g.hSpeed = 0;
            if (Math.Abs(Position.X - g.Position.X) < 6)
            {
                g.Position = Vector2.Lerp(g.Position, new Vector2(X, g.Y - 3f), 0.1f);
