...
public class ChallengeSaveData : Serializable
{
    ...
    public ChallengeSaveData Clone()
    {
    ...
    {
        BitBuffer bitBuffer = new BitBuffer(allowPacking: false);
        bitBuffer.Write(challenge);
        bitBuffer.Write((int)trophy);
        bitBuffer.Write(bestTime);
        bitBuffer.Write(targets);
        bitBuffer.Write(goodies);
        bitBuffer.Write(frameID);
        bitBuffer.Write(frameImage);
        return bitBuffer;
    }

    public static ChallengeSaveData FromBuffer(BitBuffer pBuffer)
    {
        return new ChallengeSaveData
        {
