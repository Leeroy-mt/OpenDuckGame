...
public class WireButton : Block, IWirePeripheral
{
	...
	public EditorProperty<bool> releaseOnly = new EditorProperty<bool>(val: false);

	public EditorProperty<bool> invert = new EditorProperty<bool>(val: false);

	public EditorProperty<int> orientation = new EditorProperty<int>(0, null, 0f, 3f, 1f);
	...
	public override void Update()
	{
		...
		if (invert.value)
		{
			...
			if (_sprite.frame == 1)
			{
				PhysicsObject o = Level.CheckRectAll<PhysicsObject>(_top.topLeft, _top.bottomRight).FirstOrDefault((PhysicsObject x) => !(x is TeamHat));
				if (o == null)
				{
					SFX.Play("click");
					_sprite.frame = 0;
		...
		{
			PhysicsObject o2 = Level.CheckRectAll<PhysicsObject>(_top.topLeft, _top.bottomRight).FirstOrDefault((PhysicsObject x) => !(x is TeamHat));
			if (o2 == null)
			{
