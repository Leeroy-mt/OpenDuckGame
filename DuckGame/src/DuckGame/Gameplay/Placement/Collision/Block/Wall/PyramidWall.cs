...
public class PyramidWall : Block, IBigStupidWall
{
    ...
    private Vector2 levelCenter = new Vector2(242f, 100f);
        ...
        : base(xpos, ypos)
    {
        ...
        collisionSize = new Vector2(200f, 153f);
    ...
    public void AddExtraWalls()
    {
        if (hasUp)
        {
            for (int i = -1; i < 13; i++)
            {
                Vector2 check = new Vector2(base.left + (float)(i * 16) + 12f, base.top - 4f);
        ...
        if (hasDown)
        {
            for (int j = -1; j < 13; j++)
            {
                Vector2 check2 = new Vector2(base.left + (float)(j * 16) + 12f, base.bottom + 4f);
        ...
        if (hasLeft)
        {
            for (int k = 0; k < 9; k++)
            {
                Vector2 check3 = new Vector2(base.left - 4f, base.top + (float)(k * 16) + 12f);
        ...
        for (int l = 0; l < 9; l++)
        {
            Vector2 check4 = new Vector2(base.right + 4f, base.top + (float)(l * 16) + 12f);
    ...
    public override void Draw()
    {
        ...
        if (hasUp)
        {
            for (int i = 0; i < 12; i++)
            {
                Vector2 check = new Vector2(base.left + (float)(i * 16) + 12f, base.top - 2f);
                Graphics.DrawRect(check + new Vector2(-2f, -2f), check + new Vector2(2f, 2f), Color.Orange, 1f);
            }
        }
        if (hasDown)
        {
            for (int j = -1; j < 13; j++)
            {
                Vector2 check2 = new Vector2(base.left + (float)(j * 16) + 12f, base.bottom + 2f);
                Graphics.DrawRect(check2 + new Vector2(-2f, -2f), check2 + new Vector2(2f, 2f), Color.Orange, 1f);
            }
        }
        if (hasLeft)
        {
            for (int k = 0; k < 9; k++)
            {
                Vector2 check3 = new Vector2(base.left - 2f, base.top + (float)(k * 16) + 12f);
                Graphics.DrawRect(check3 + new Vector2(-2f, -2f), check3 + new Vector2(2f, 2f), Color.Orange, 1f);
            }
        }
        if (hasRight)
        {
            for (int l = 0; l < 9; l++)
            {
                Vector2 check4 = new Vector2(base.right + 2f, base.top + (float)(l * 16) + 12f);
                Graphics.DrawRect(check4 + new Vector2(-2f, -2f), check4 + new Vector2(2f, 2f), Color.Orange, 1f);
            }
        }
    }
}
