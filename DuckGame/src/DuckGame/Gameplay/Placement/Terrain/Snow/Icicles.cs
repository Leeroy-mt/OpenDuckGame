...
public class Icicles : MaterialThing
{
    ...
    public Icicles(float xpos, float ypos, int dir)
        : base(xpos, ypos)
    {
        style = new EditorProperty<int>(0, this, 0f, 3f, 1f);
    ...
    {
        if (with.impactPowerV > 2f && (graphic as SpriteMap).frame != 3)
        {
            Break(with.isLocal);
        }
        else if (with is Net || with is Dart || with is CampingBall || with is Sword || with is EnergyScimitar)
        {
            Break(with.isLocal);
        }
