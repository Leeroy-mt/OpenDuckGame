	...
	public float textToSpeechRate
	{
		...
		set
		{
			if (!Program.isLinux && _textToSpeechRate != value && textToSpeech)
			{
				...
				{
					SFX.StopSaying();
					SFX.Say("Speed " + (int)Math.Round((value - 0.5f) * 20f));
				}
			}
	...
	public int textToSpeechVoice
	{
		...
		set
		{
			if (!Program.isLinux && _textToSpeechVoice != value && textToSpeech)
			{
				...
				if (value < 0 || value >= voices.Count)
				{
					value = 0;
				}
				SFX.SetSayVoice(voices[value]);
				if (MonoMain.started)
				{
					SFX.StopSaying();
					SFX.Say(voices[value]);
				}
			}
