...
public class Noise
{
	...
	public static float Generate(float x, float y, float z)
	{
		...
		float x4 = x3 - (float)i1 + 1f / 6f;
		float y3 = y2 - (float)j2 + 1f / 6f;
		float z3 = z2 - (float)k2 + 1f / 6f;
		float x5 = x3 - (float)i2 + 1f / 3f;
		...
		int jj = Mod(j, 256);
		int kk = Mod(k, 256);
		float t2 = 0.6f - x3 * x3 - y2 * y2 - z2 * z2;
		float n0;
		if (t2 < 0f)
		{
			n0 = 0f;
		...
		{
			t2 *= t2;
			n0 = t2 * t2 * grad(perm[ii + perm[jj + perm[kk]]], x3, y2, z2);
		}
		float t3 = 0.6f - x4 * x4 - y3 * y3 - z3 * z3;
		float n1;
		if (t3 < 0f)
		{
			n1 = 0f;
		...
		{
			t3 *= t3;
			n1 = t3 * t3 * grad(perm[ii + i1 + perm[jj + j2 + perm[kk + k2]]], x4, y3, z3);
		}
		float t4 = 0.6f - x5 * x5 - y4 * y4 - z4 * z4;
		float n2;
		if (t4 < 0f)
		{
			n2 = 0f;
		...
		{
			t4 *= t4;
			n2 = t4 * t4 * grad(perm[ii + i2 + perm[jj + j3 + perm[kk + k3]]], x5, y4, z4);
		}
		float t5 = 0.6f - x6 * x6 - y5 * y5 - z5 * z5;
		float n3;
		if (t5 < 0f)
		{
			n3 = 0f;
		...
		{
			t5 *= t5;
			n3 = t5 * t5 * grad(perm[ii + 1 + perm[jj + 1 + perm[kk + 1]]], x6, y5, z5);
		}
