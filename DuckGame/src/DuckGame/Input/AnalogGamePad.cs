...
public class AnalogGamePad : InputDevice
{
    ...
    {
        if (mapping == 16 && _startPressed)
        {
            return true;
        }
        if (any)
        {
            if ((_state.buttons & ~_statePrev.buttons) != 0)
            {
                return true;
            }
            return false;
        }
        if (_state.IsButtonDown((PadButton)mapping) && !_statePrev.IsButtonDown((PadButton)mapping))
        {
            return true;
        }
        return false;
    ...
    public override bool MapDown(int mapping, bool any = false)
    {
        if (any)
        {
            foreach (PadButton butt in _xboxButtons)
            {
                if (_state.IsButtonDown(butt))
                {
                    return true;
                }
            }
        ...
        if (_state.IsButtonDown((PadButton)mapping))
        {
            return true;
        }
        return false;
