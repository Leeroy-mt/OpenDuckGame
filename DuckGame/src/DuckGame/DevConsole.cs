...
public class DevConsole
{
  ...
  //  public static void Draw2() //fna batch test
  //  {
		//if (Layer.core._console != null)
		//{
		//	Layer.core._console.camera.width = Resolution.current.x / 2;
		...
		//int numSectionsVert = (int)(Layer.core._console.camera.height * dimensions.y / (16f * _tray.scale.y)) - 2;
		//int numSectionsHor = (int)(Layer.core._console.camera.width * dimensions.x / (16f * _tray.scale.x)) - 2;
		//_tray.position = default;
		//Graphics.Draw(_tray, new Rectangle(0f, 0f, 18f, 18f));
		//_tray.y = 18f * _tray.scale.y + (float)numSectionsVert * (16f * _tray.scale.y);
  //      Graphics.Draw(_tray, new Rectangle(0f, _tray.height - 18, 18f, 18f));
		//_tray.x = 18f * _tray.scale.x + (float)(numSectionsHor - 6) * (16f * _tray.scale.x);
  //      Graphics.Draw(_tray, new Rectangle(_tray.width - 114, _tray.height - 18, 114f, 18f));
		//for (int i = 0; i < numSectionsHor; i++)
		//{
		//	_tray.x = 18f * _tray.scale.x + 16f * _tray.scale.x * (float)i;
		//	_tray.y = 0;
  //          Graphics.Draw(_tray, new Rectangle(16f, 0f, 16f, 18f));
		//	if (i < numSectionsHor - 6)
		//	{
		//		_tray.x = 18f * _tray.scale.x + 16f * _tray.scale.x * (float)i;
		//		_tray.y = 18f * _tray.scale.y + (float)numSectionsVert * (16f * _tray.scale.y);
  //              Graphics.Draw(_tray, new Rectangle(16f, _tray.height - 18, 16f, 18f));
		//	}
		//}
		//_tray.x = 18f * _tray.scale.x + (float)numSectionsHor * (16f * _tray.scale.x);
		//_tray.y = 0;
  //      Graphics.Draw(_tray, new Rectangle(_tray.width - 18, 0f, 18f, 18f));
		//for (int j = 0; j < numSectionsVert; j++)
		//{
		//	_tray.x = 0;
		//	_tray.y = 18f * _tray.scale.y + 16f * _tray.scale.y * (float)j;
  //          Graphics.Draw(_tray, new Rectangle(0f, 18f, 18f, 16f));
		//	_tray.x = 18f * _tray.scale.x + (float)numSectionsHor * (16f * _tray.scale.x);
		//	_tray.y = 18f * _tray.scale.y + 16f * _tray.scale.y * (float)j;
  //          Graphics.Draw(_tray, new Rectangle(_tray.width - 18, 18f, 18f, 16f));
		//}
		//Graphics.DrawRectangleFilled(
		//	new(Vec2.Zero, new Vec2(18f * _tray.scale.x + (float)numSectionsHor * (16f * _tray.scale.x) + _tray.scale.y * 4f, (float)(numSectionsVert + 2) * (16f * _tray.scale.y))),
		//	Color.Black * 0.8f * _core.alpha,
		//	0.7f);
		//_core.fancyFont.scale = new Vec2(_tray.scale.x / 2f);
		//_core.fancyFont.depth = 0.98f;
		//_core.fancyFont.alpha = _core.alpha;
		...
		//float width = (float)(numSectionsHor + 2) * (16f * _tray.scale.x);
		//string ver = DG.version;
		//_core.fancyFont.Draw2(ver, 82f * _tray.scale.x + (float)(numSectionsHor - 6) * (16f * _tray.scale.x), height + 7f * _tray.scale.y, new Color(62, 114, 122), 0.98f);
		//_core.cursorPosition = Math.Min(Math.Max(_core.cursorPosition, 0), _core.typing.Length);
		//if (_raster != null)
		//{
		//	_raster.scale = new Vec2(0.5f);
		//	_raster.alpha = _core.alpha;
		//	_raster.Draw2(_core.typing, 4f * _tray.scale.x, height + _tray.scale.y * 8f - (float)_raster.characterHeight * _raster.scale.y / 2f, Color.White, 0.9f);
		//	Vec2 vec = new Vec2(_raster.GetWidth(_core.typing.Substring(0, _core.cursorPosition)) + 4f * _tray.scale.x + 1f, height + 6f * _tray.scale.y);
		//	Graphics.DrawLIne(vec, vec + new Vec2(0f, 4f * _tray.scale.x), Color.White, 1f, 1f);
		//}
		//else
		...
		//	_core.font.scale = new Vec2(_tray.scale.x / 2f);
		//	_core.font.alpha = _core.alpha;
		//	_core.font.Draw2(_core.typing, 4f * _tray.scale.x, height + 6f * _tray.scale.y, Color.White, 0.9f);
		//	Vec2 vec2 = new Vec2(_core.font.GetWidth(_core.typing.Substring(0, _core.cursorPosition)) + 4f * _tray.scale.x, height + 6f * _tray.scale.y);
		//	Graphics.DrawLIne(vec2, vec2 + new Vec2(0f, 4f * _tray.scale.x), Color.White, 2f, 1f);
		//}
		//int index = _core.lines.Count - 1 - _core.viewOffset;
		...
		//		_raster.singleLine = true;
		//		_raster.enforceWidthByWord = false;
		//		_raster.Draw2(lineNumber, 4f * _tray.scale.x, height - vOffset + 2f, (index % 2 > 0) ? (Color.Gray * 0.4f) : (Color.Gray * 0.6f), 0.9f);
		//		_raster.Draw2(line.SectionString() + line.line, 4f * _tray.scale.x + numWidth, height - vOffset + 2f, line.color, 0.9f);
		//		vOffset += lineHeight;
		//	}
		//	else
		//	{
		...
		//		_core.font.singleLine = true;
		//		_core.font.enforceWidthByWord = false;
		//		_core.font.Draw2(lineNumber, 4f * _tray.scale.x, height - 18f * mul - vOffset + 2f, (index % 2 > 0) ? (Color.Gray * 0.4f) : (Color.Gray * 0.6f), 0.9f);
		//		_core.font.Draw2(line.SectionString() + line.line, 4f * _tray.scale.x + numWidth, height - 18f * mul - vOffset + 2f, line.color * 0.8f, 0.9f);
		//		vOffset += 18f * mul;
		//	}
		//	index--;
		//}
