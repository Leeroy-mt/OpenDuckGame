...
public class MonoMain : Game
{
	...
	private void DoLazyLoading()
	{
		...
		{
			lazyLoadActions.Dequeue()?.Invoke();
		}
	}
	...
	private void AddLoadingAction(Action pAction)
	{
		_thingsToLoad.Enqueue(pAction);
	}
	...
	private void SetStarted()
	{
		...
		{
			_lazyLoadThread = new Thread(DoLazyLoading);
			_lazyLoadThread.CurrentCulture = CultureInfo.InvariantCulture;
			_lazyLoadThread.Priority = ThreadPriority.BelowNormal;
			_lazyLoadThread.IsBackground = true;
	...
	public void RunUpdate(GameTime gameTime)
	{
		...
		if (_thingsToLoad.Count > 0 && (!Program.isLinux || _waitToStartLoadingTimer.elapsed.TotalMilliseconds > 3500.0))
		{
			_loadTimer.Restart();
			while (_thingsToLoad.Count > 0 && _loadTimer.elapsed.TotalMilliseconds < 40.0)
			{
				currentActionQueue = _thingsToLoad;
				LoadingAction a = _thingsToLoad.Peek();
				if (a.Invoke())
				{
	...
	protected void RunDraw(GameTime gameTime)
	{
		...
		if (!_started)
		{
			...
			Graphics.DrawRect(loadBarPos, loadBarPos + new Vec2(loadBarSize.x * loaded, loadBarSize.y), Color.White * 0.1f, 0.6f);
