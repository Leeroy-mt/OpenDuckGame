...
public class InputProfile
{
    ...
    private static string[] _defaultPlayerMappingStrings = new string[8] { "MPPlayer1", "MPPlayer2", "MPPlayer3", "MPPlayer4", "MPPlayer5", "MPPlayer6", "MPPlayer7", "MPPlayer8" };
    ...
    private InputDevice _defaultLastActiveDevice = new InputDevice();
    ...
    public static string MPPlayer1 => _defaultPlayerMappingStrings[0];

    public static string MPPlayer2 => _defaultPlayerMappingStrings[1];

    public static string MPPlayer3 => _defaultPlayerMappingStrings[2];

    public static string MPPlayer4 => _defaultPlayerMappingStrings[3];

    public static string MPPlayer5 => _defaultPlayerMappingStrings[4];

    public static string MPPlayer6 => _defaultPlayerMappingStrings[5];

    public static string MPPlayer7 => _defaultPlayerMappingStrings[6];

    public static string MPPlayer8 => _defaultPlayerMappingStrings[7];

    public static string[] MPPlayers => _defaultPlayerMappingStrings;
    ...
    public static List<InputProfile> defaultProfiles => _core.defaultProfiles;
    ...
    public static InputProfile DefaultPlayer1 => _core.DefaultPlayer1;

    public static InputProfile DefaultPlayer2 => _core.DefaultPlayer2;

    public static InputProfile DefaultPlayer3 => _core.DefaultPlayer3;

    public static InputProfile DefaultPlayer4 => _core.DefaultPlayer4;

    public static InputProfile DefaultPlayer5 => _core.DefaultPlayer5;

    public static InputProfile DefaultPlayer6 => _core.DefaultPlayer6;

    public static InputProfile DefaultPlayer7 => _core.DefaultPlayer7;

    public static InputProfile DefaultPlayer8 => _core.DefaultPlayer8;
    ...
    public static Dictionary<string, InputProfile> profiles => _core._profiles;
    ...
    public ushort prevState => _prevState;
    ...
    protected void StickPress(PadButton b, bool press)
    {
        ...
        else if (_leftStickStates[b] == InputState.Down || _leftStickStates[PadButton.DPadLeft] == InputState.Pressed)
        {
    ...
    {
        foreach (InputDevice key in _mappings.Keys)
        {
            if (key.isConnected)
            {
                return true;
    ...
    public virtual Sprite GetTriggerImage(string trigger)
    {
        ...
        if (mapping == -1)
        {
            foreach (KeyValuePair<InputDevice, MultiMap<string, int>> pair in _mappings)
            {
                ...
                if (mapping != -1)
                {
                    return pair.Key.DoGetMapImage(mapping);
                }
            }
            return null;
        }
        return last_device.DoGetMapImage(mapping);
    }
    ...
    public MultiMap<string, int> GetControllerMap<T>() where T : InputDevice
    {
        ...
        InputDevice d = Input.GetDevice<T>();
    ...
    public InputProfile Clone()
    {
        ...
        clone._name = name;
        clone._state = _state;
        clone._prevState = _prevState;
        clone.dindex = dindex;
        clone.swapBack = swapBack;
        foreach (KeyValuePair<InputDevice, MultiMap<string, int>> kvp in _mappings)
        {
    ...
    public virtual bool Pressed(string trigger, bool any = false)
    {
        ...
        foreach (KeyValuePair<InputDevice, MultiMap<string, int>> map in _mappings)
        {
            ...
            if (any)
            {
                if (mapped_device.MapPressed(-1, any: true))
                {
            ...
            {
                if (!map.Value.TryGetValue(trigger, out var vals))
                {
                    continue;
                }
    ...
    public virtual bool Released(string trigger)
    {
        ...
        {
            InputDevice mapped_device = map.Key;
            if ((_virtualInput != null && !(mapped_device is VirtualInput)) || !map.Value.TryGetValue(trigger, out var vals))
            {
                continue;
            }
            foreach (int val in vals)
    ...
    public void UpdateTriggerStates()
    {
        ...
        if (_repeat)
        {
            if (Pressed("MENULEFT") || Pressed("MENURIGHT") || Pressed("MENUUP") || Pressed("MENUDOWN"))
            {
                if (!_repeating)
                {
                    _repeatTime = 1.8f;
