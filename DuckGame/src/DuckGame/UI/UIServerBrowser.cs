...
public class UIServerBrowser : UIMenu
{
    ...
    private const long MIN_FRAMES_BETWEEN_SEARCHES = 0L;
            ...
            if (loadedMods != null && loadedMods != "")
            {
                ...
                foreach (string s in array)
                {
                    ...
                    if (s2.Length == 2)
                    {
                        ...
                        {
                            DevConsole.Log(DCSection.General, "SubscribeAndRestart failed to enable workshop item (" + s.ToString() + ")");
                        }
                    }
                }
            }
        ...
        : base(title, xpos, ypos, wide, high, "@WASD@@SELECT@JOIN @MENU1@REFRESH @CANCEL@BACK", conProfile)
    {
        ...
        _maxLobbiesToShow = 8;
        _box = new UIBox(0f, 0f, -1f, _maxLobbiesToShow * 36, vert: true, isVisible: false);
        Add(_box);
        _fancyFont = new FancyBitmapFont("smallFont");
        _fancyFont.maxWidth = (int)base.width - 100;
        _fancyFont.maxRows = 2;
        scrollBarOffset = 0;
        _editModMenu = new UIMenu("<mod name>", Layer.HUD.camera.width / 2f, Layer.HUD.camera.height / 2f, 160f, -1f, "@SELECT@SELECT");
    ...
    private void UpdateLobbySearch()
    {
        ...
        else
        {
            for (int i = 0; i < lobbies; i++)
            {
                ...
                if (_lobbies.FirstOrDefault((LobbyData x) => x.lobby != null && x.lobby.id == lobby.id) != null)
                {
                    continue;
                }
                ...
                LobbyData d2 = new LobbyData();
                d2.lobby = lobby;
                d2.name = DuckNetwork.core.FilterText(lobbyName, null);
                d2.hasCustomName = lobby.GetLobbyData("customName") == "true";
                d2.modHash = lobby.GetLobbyData("modhash");
                d2.requiredWins = lobby.GetLobbyData("requiredwins");
                d2.restsEvery = lobby.GetLobbyData("restsevery");
                d2.wallMode = lobby.GetLobbyData("wallmode");
                d2.customLevels = lobby.GetLobbyData("customLevels");
                d2.version = lobby.GetLobbyData("version");
                d2.started = lobby.GetLobbyData("started");
                d2.type = lobby.GetLobbyData("type");
                try
                {
        ...
        else if (base.open)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    _scrollItemOffset = (int)((float)(_lobbies.Count - _maxLobbiesToShow) * heightScrolled);
                }
            ...
            if (_scrollItemOffset != 0)
            {
                scrollBarOffset = (int)Lerp.FloatSmooth(0f, scrollBarScrollableHeight, (float)_scrollItemOffset / (float)(_lobbies.Count - _maxLobbiesToShow));
            }
            else
            {
                scrollBarOffset = 0;
    ...
    public override void Draw()
    {
        ...
        if (base.open)
        {
            ...
            if (_lobbies.Count == 0)
            {
                ...
                {
                    _fancyFont.Draw("No games found!", new Vec2(boxLeft + 10f, boxTop + 2f), Color.Yellow, 0.5f);
                }
                else
                {
                    _fancyFont.Draw("Waiting for game list.", new Vec2(boxLeft + 10f, boxTop + 2f), Colors.DGGreen, 0.5f);
                }
            }
            if (mode != SearchMode.None)
            {
                float boxLeft2 = _box.x - _box.halfWidth + 116f;
                float boxTop2 = _splitter.topSection.y - 5f;
                _refreshingDots += 0.01f;
                if (_refreshingDots > 1f)
                {
                    _refreshingDots = 0f;
                }
                ...
                for (int i = 0; i < 3; i++)
                {
                    if (_refreshingDots * 4f > (float)(i + 1))
                    {
                        refreshString += ".";
                    }
                }
            ...
            for (int j = 0; j < _maxLobbiesToShow; j++)
            {
                ...
                float boxTop3 = _box.y - _box.halfHeight + (float)(36 * j);
                if (_hoverIndex == lobbyIndex)
                {
                    Graphics.DrawRect(new Vec2(boxLeft3, boxTop3), new Vec2(boxLeft3 + _box.width - 14f, boxTop3 + 36f), Color.White * 0.6f, 0.4f);
                }
                else if ((lobbyIndex & 1) != 0)
                {
                    Graphics.DrawRect(new Vec2(boxLeft3, boxTop3), new Vec2(boxLeft3 + _box.width - 14f, boxTop3 + 36f), Color.White * 0.1f, 0.4f);
                }
                ...
                if (lobby.workshopItems.Count > 0)
                {
                    ...
                    if (w.data != null)
                    {
                        lobby.workshopItems = lobby.workshopItems.OrderByDescending((WorkshopItem x) => (x.data != null) ? x.data.votesUp : 0).ToList();
                        if (!lobby.downloadedWorkshopItems)
                        {
                            ...
                            {
                                ulong id = item.id;
                                if (ModLoader.accessibleMods.FirstOrDefault((Mod x) => x.configuration.workshopID == id) == null)
                                {
                                    if (firstIteration)
                                    {
                        ...
                        modDetails += "\n|GRAY|";
                        if (!_previewMap.ContainsKey(w.id))
                        {
                ...
                {
                    modDetails = modDetails + "First to " + lobby.requiredWins.ToString() + " ";
                }
                if (!string.IsNullOrWhiteSpace(lobby.restsEvery))
                {
                    modDetails = modDetails + "rests every " + lobby.restsEvery.ToString() + ". ";
                }
                ...
                {
                    modDetails = modDetails + "Custom Levels: " + lobby.customLevels.ToString() + ". ";
                }
                if (!string.IsNullOrWhiteSpace(lobby.hasModifiers) && lobby.hasModifiers != "false")
                {
                    modDetails += "Modifiers: ACTIVE.";
                }
                Graphics.DrawRect(new Vec2(boxLeft3 + 2f, boxTop3 + 2f), new Vec2(boxLeft3 + 36f - 2f, boxTop3 + 36f - 2f), Color.Gray, 0.5f, filled: false, 2f);
                if (workshopTextures.Count > 0)
                {
                    ...
                    for (int iTex = 0; iTex < 4; iTex++)
                    {
                        ...
                        {
                            _noImage.scale = new Vec2(16f / (float)_noImage.texture.width);
                        }
                        else
                        {
                            _noImage.scale = new Vec2(32f / (float)_noImage.texture.width);
                        }
                        if (_noImage.texture.width != _noImage.texture.height)
                        {
                            if (_noImage.texture.width > _noImage.texture.height)
                            {
                                _noImage.scale = new Vec2(32f / (float)_noImage.texture.height);
                                Graphics.Draw(_noImage, boxLeft3 + 2f + drawOffset.x, boxTop3 + 2f + drawOffset.y, new Rectangle(_noImage.texture.width / 2 - _noImage.texture.height / 2, 0f, _noImage.texture.height, _noImage.texture.height), 0.5f);
                            }
                            else
                            {
                                Graphics.Draw(_noImage, boxLeft3 + 2f + drawOffset.x, boxTop3 + 2f + drawOffset.y, new Rectangle(0f, 0f, _noImage.texture.width, _noImage.texture.width), 0.5f);
                            }
                        }
                        else
                        {
                            Graphics.Draw(_noImage, boxLeft3 + 2f + drawOffset.x, boxTop3 + 2f + drawOffset.y, 0.5f);
                        }
                        drawOffset.x += 16f;
                        if (drawOffset.x >= 32f)
                        {
                ...
                {
                    Graphics.Draw(_noImage, boxLeft3 + 2f, boxTop3 + 2f, 0.5f);
                }
                ...
                {
                    titleString += " |DGRED|(";
                    titleString = ((lobby.version != DG.version) ? (DuckNetwork.CheckVersion(lobby.version) switch
                    {
                    ...
                    }) : ((lobby.datahash != Network.gameDataHash) ? (titleString + "Their version is incompatible.") : ((lobby.started == "true") ? (titleString + "This game is in progress.") : ((lobby.userCount >= lobby.numSlots) ? (titleString + "Lobby is full.") : ((lobby.lobby != null && lobby.type != "2") ? (titleString + "This game is not public.") : (lobby.hasLocalMods ? (titleString + "This game is using non-workshop mods.") : ((!(lobby.customLevels != "") || !(lobby.customLevels != "0") || !ParentalControls.AreParentalControlsActive()) ? (titleString + "Cannot join.") : (titleString + "This game has blocked content."))))))));
                    titleString += ")";
                    Graphics.DrawRect(new Vec2(boxLeft3, boxTop3), new Vec2(boxLeft3 + _box.width - 14f, boxTop3 + 36f), Color.Black * 0.5f, 0.99f);
                }
                _fancyFont.maxWidth = 1000;
                float passOffset = 0f;
                if (lobby.hasPassword)
                {
                    Graphics.Draw(_lockedServer, boxLeft3 + 36f + 10f, boxTop3 + 2.5f, 0.5f);
                    passOffset += 10f;
                }
                if (lobby.hasCustomName)
                {
                    Graphics.Draw(_namedServer, boxLeft3 + passOffset + 36f + 10f, boxTop3 + 2.5f, 0.5f);
                    passOffset += 10f;
                }
                if (lobby.isGlobalLobby)
                {
                    Graphics.Draw(_globeIcon, boxLeft3 + passOffset + 36f + 10f, boxTop3 + 2.5f, 0.5f);
                    passOffset += 10f;
                }
                _fancyFont.Draw(titleString, new Vec2(boxLeft3 + 36f + passOffset + 10f, boxTop3 + 2f), Color.Yellow, 0.5f);
                if (lobby.version == DG.version)
                {
                    _fancyFont.Draw(lobby.version, new Vec2(boxLeft3 + 430f + 10f, boxTop3 + 2f), Colors.DGGreen * 0.45f, 0.5f);
                }
                else
                {
                    _fancyFont.Draw(lobby.version, new Vec2(boxLeft3 + 430f + 10f, boxTop3 + 2f), Colors.DGRed * 0.45f, 0.5f);
                }
                _fancyFont.Draw("|WHITE|Ping:", new Vec2(boxLeft3 + 440f, boxTop3 + 26f), Color.White * 0.45f, 0.5f);
                ...
                if (lobby.estimatedPing != -1)
                {
                    ...
                    _fancyFont.Draw(lobby.estimatedPing + "ms", new Vec2(boxLeft3 + 470f, boxTop3 + 26f), c * 0.45f, 0.5f);
                }
                else
                {
                    _fancyFont.Draw("????ms", new Vec2(boxLeft3 + 470f, boxTop3 + 26f), Colors.DGRed * 0.45f, 0.5f);
                }
                if (lobby.lobby != null)
                {
                    Graphics.Draw(_steamIcon, boxLeft3 + 36f, boxTop3 + 2.5f, 0.5f);
                }
                else
                {
                    Graphics.Draw(_lanIcon, boxLeft3 + 36f, boxTop3 + 2.5f, 0.5f);
                }
                _fancyFont.Draw(modDetails, new Vec2(boxLeft3 + 36f, boxTop3 + 6f + (float)_fancyFont.characterHeight), Color.LightGray, 0.5f);
            }
