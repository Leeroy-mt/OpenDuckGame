...
public class UIControlConfig : UIMenu
{
	...
	private List<string> inputTypes = new List<string> { "GAMEPAD", "KEYBOARD" };
	...
	public void SwitchPlayerProfile()
	{
		...
		{
			List<string> checkedPads = new List<string>();
			for (int j = 0; j < 8; j++)
			{
				...
				string padName = DInput.GetProductName(j);
				string padGUID = DInput.GetProductGUID(j);
				string fullIdentifier = padName + padGUID;
				if (!checkedPads.Contains(fullIdentifier))
				{
					checkedPads.Add(fullIdentifier);
					inputMaps.Add(Input.GetDefaultMapping(padName, padGUID).Clone());
					if (padName.Length > 24)
					{
						padName = padName.Substring(0, 24);
					}
					inputTypes.Add(padName);
				}
			}
		}
	...
	public UIControlConfig(UIMenu openOnClose, string title, float xpos, float ypos, float wide = -1f, float high = -1f, string conString = "", InputProfile conProfile = null)
		: base(title, xpos, ypos, wide, high, conString, conProfile)
	{
		_openOnClose = openOnClose;
		new List<string> { "P1   ", "P2   ", "P3   ", "P4" };
		new List<string> { "GAMEPAD", "KEYBOARD", "PAD + KEYS" };
		BitmapFont littleFont = new BitmapFont("smallBiosFontUI", 7, 5);
		UIBox box = new UIBox(vert: true, isVisible: false);
		_configuringToggle = new UIMenuItemToggle("", new UIMenuActionCallFunction(SwitchConfigType), new FieldBinding(this, "inputConfigType"), default(Color), null, inputTypes, compressedMulti: true, tiny: true);
		box.Add(_configuringToggle);
		UIText t = new UIText(" ", Color.White);
		...
		box.Add(_controlElements[_controlElements.Count - 1]);
		...
		box.Add(_controlElements[_controlElements.Count - 1]);
		box.Add(new UIText(" ", Color.White, UIAlign.Center, -6f));
		...
		box.Add(_controlElements[_controlElements.Count - 1]);
		UIMenuItem menuItem = new UIMenuItem("|RED|REVERT TO DEFAULT", new UIMenuActionCallFunction(ResetToDefault));
		...
		Add(_playerBoxes[0]);
		_confirmMenu = new UIMenu("SAVE CHANGES?", Layer.HUD.camera.width / 2f, Layer.HUD.camera.height / 2f, 160f, -1f, "@SELECT@SELECT @CANCEL@BACK");
		...
		_confirmMenu.Close();
		_warningMenu = new UIMenu("WARNING!", Layer.HUD.camera.width / 2f, Layer.HUD.camera.height / 2f, 180f, -1f, "@SELECT@ I see...");
		_warningMenu.Add(new UIText("", Color.White, UIAlign.Center, -3f)
		{
			scale = new Vec2(0.5f)
		});
		_warningMenu.Add(new UIText("One or more profiles have", Color.White, UIAlign.Center, -4f)
		{
			scale = new Vec2(0.5f)
		});
		_warningMenu.Add(new UIText("|DGBLUE|custom controls|PREV| defined, which will", Color.White, UIAlign.Center, -4f)
		{
			scale = new Vec2(0.5f)
		});
		_warningMenu.Add(new UIText("|DGRED|override|PREV| any controls set here!", Color.White, UIAlign.Center, -4f)
		{
			scale = new Vec2(0.5f)
		});
		_warningMenu.Add(new UIText("", Color.White, UIAlign.Center, -3f)
		...
		_warningMenu.Add(new UIText("If these controls are not working,", Color.White, UIAlign.Center, -4f)
		{
			scale = new Vec2(0.5f)
		});
		_warningMenu.Add(new UIText("enter the hat console in the lobby", Color.White, UIAlign.Center, -4f)
		{
			scale = new Vec2(0.5f)
		});
		_warningMenu.Add(new UIText("and press |DGORANGE|EDIT|PREV| on your profile name.", Color.White, UIAlign.Center, -4f)
		{
			scale = new Vec2(0.5f)
		});
		_warningMenu.Add(new UIText("Select |DGORANGE|CONTROLS|PREV|, select your desired", Color.White, UIAlign.Center, -4f)
		{
			scale = new Vec2(0.5f)
		});
		_warningMenu.Add(new UIText("input device, go to |DGORANGE|PAGE 2|PREV|", Color.White, UIAlign.Center, -4f)
		{
			scale = new Vec2(0.5f)
		});
		_warningMenu.Add(new UIText("and select |DGORANGE|RESET|PREV|.", Color.White, UIAlign.Center, -4f)
		{
			scale = new Vec2(0.5f)
		});
		_warningMenu.Add(new UIText("", Color.White, UIAlign.Center, -3f)
	...
	public override void Update()
	{
		if (base.open)
		{
			if (!UIMenu.globalUILock && showWarning)
			{
