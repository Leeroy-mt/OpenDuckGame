...
public static class Program
{
		...
		try
		{
			if (e is UnauthorizedAccessException)
			{
				...
				if (pathStart > 0)
				{
					...
					if (pathEnd > 0)
					{
						FileAttributes attr = new FileInfo(u.Message.Substring(pathStart, pathEnd - pathStart)).Attributes;
						string details = "(File is ";
						int num = 0;
						foreach (FileAttributes f in Enum.GetValues(typeof(FileAttributes)))
						{
							if ((attr & f) > (FileAttributes)0)
							{
								if (num > 0)
								{
									details += ",";
	...
	public static void WriteToLog(string s, bool modRelated)
	{
		...
		{
			StreamWriter streamWriter2 = new StreamWriter("ducklog.txt", append: true);
	...
	{
		StreamWriter file = new StreamWriter("netlog.txt", append: false);
	...
	private static void UnhandledExceptionTrapperTestServer(object sender, UnhandledExceptionEventArgs e)
	{
		string error = MonoMain.GetExceptionString(e);
		StreamWriter streamWriter = new StreamWriter("ducklog.txt", append: true);
