...
public class GlobalData : DataClass
{
	...
	public int GetHatMatchWins(string hat)
	{
		if (hatWins.ContainsKey(hat))
		{
			return hatWins[hat];
		}
		return 0;
	...
	public GlobalData()
	{
		...
		{
			if (info.FieldType == typeof(StatBinding))
			{
				StatBinding newBinding = new StatBinding();
				newBinding.BindName(info.Name);
				info.SetValue(this, newBinding);
			}
		}
	}
}
