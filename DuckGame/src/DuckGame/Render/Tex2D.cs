...
public class Tex2D : Tex2DBase
{
	...
	public Tex2D(int width, int height)
		: base("__internal", 0)
	{
		_base = new Texture2D(Graphics.device, width, height, mipMap: false, SurfaceFormat.Color);
		_frameWidth = width;
		_frameHeight = height;
	...
	public override Color[] GetData()
	{
		if (_base != null)
		{
			Color[] colors = new Color[_base.Width * _base.Height];
			_base.GetData(colors);
			return colors;
		}
		return null;
	}
	...
	public static implicit operator Texture2D(Tex2D tex)
	{
		return tex._base;
	}
