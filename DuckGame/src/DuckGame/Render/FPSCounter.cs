...
internal static class FPSCounter
{
    ...
    {
        if (!_fpsIncrement.ContainsKey(index))
        {
            _fpsValue[index] = 0;
            _fpsIncrement[index] = 0;
            _fpsTimer[index] = new Stopwatch();
    ...
    {
        if (_fpsValue.ContainsKey(index))
        {
            return _fpsValue[index];
        }
        return 0;
    }

    public static void Render(GraphicsDevice graphics, float xpos = 16f, float ypos = 16f, int index = -1, string label = "FPS")
    {
        Render(graphics, Color.White, xpos, ypos, index, label);
    }

    public static void Render(GraphicsDevice graphics, Color color, float xpos = 16f, float ypos = 16f, int index = -1, string label = "FPS")
    {
        if (index == -1)
        {
            Tick(index);
    ...
    public static void DrawText(GraphicsDevice graphics, Color color, string text, float xpos = 16f, float ypos = 16f)
    {
        if (!_initialized)
        {
            Initialize(graphics);
        ...
        _batch.Begin(SpriteSortMode.Immediate, BlendState.AlphaBlend, SamplerState.PointClamp, DepthStencilState.None, RasterizerState.CullNone, null, Matrix.Identity);
        _biosFont.Draw(_batch, text, xpos, ypos, color);
        _batch.End();
    }
}
