...
public class Layer : DrawList
{
	...
	//SpriteBatch Batch;//fna batch test
	...
	public Layer(string nameval, int depthval = 0, Camera cam = null, bool targetLayer = false, Vec2 targetSize = default(Vec2))
	{
        ...
        //Batch = new(Graphics.device); //fna batch test
        _state = new RasterizerState();
		_state.CullMode = CullMode.None;
		_camera = cam;
	...
	public virtual void Begin(bool transparent, bool isTargetDraw = false)
	{
		...
		if (target != null && isTargetDraw)
		{
			...
			//Batch.Begin(SpriteSortMode.BackToFront, blendState, SamplerState.PointClamp, _targetDepthStencil, _state, effect, c.getMatrix()); //fna batch test
			c.position = realPos;
            c.size = realSize;
		}
		else if (blurry || _blurEffect)
		{
			if (!transparent)
			{
                ...
                //Batch.Begin(SpriteSortMode.FrontToBack, blendState, SamplerState.LinearClamp, DepthStencilState.Default, _state, effect, c.getMatrix()); //fna batch test
            }
            else
			{
                ...
                //Batch.Begin(SpriteSortMode.BackToFront, blendState, SamplerState.LinearClamp, DepthStencilState.DepthRead, _state, effect, c.getMatrix()); //fna batch test
            }
        }
		else if (!transparent)
		{
            ...
            //Batch.Begin(SpriteSortMode.FrontToBack, blendState, SamplerState.PointClamp, DepthStencilState.Default, _state, effect, c.getMatrix()); //fna batch test
        }
        else
		{
            ...
            //Batch.Begin(SpriteSortMode.BackToFront, blendState, SamplerState.PointClamp, DepthStencilState.DepthRead, _state, effect, c.getMatrix()); //fna batch test
        }
    }
