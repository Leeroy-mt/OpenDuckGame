...
namespace XnaToFna.ProxyDInput
{
    public static class DInput
    {
        ...
        public static void Update()
        {
            for (int index = 0; index < States.Length; ++index)
            {
                var playerIndex = (PlayerIndex)index;
                GamePadState state1 = GamePad.GetState(playerIndex, GamePadDeadZone.IndependentAxes);
                //GamePadState state1 = GamePad.GetState(index, GamePadDeadZone.IndependentAxes); old
        ...
        public static string GetProductGUID(int player) => player < States.Length ? GamePad.GetGUIDEXT((PlayerIndex)player) : string.Empty;
    }
}
